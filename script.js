...
const selectedItemDatas = document.getElementById("selected-item-datas");
const selectedItemId = document.getElementById("selected-tower-id");
    ...
    onClick() {
        ...
        if (this.isLocked == false && userHasPickedTower == true) {
            ...
            if (towers[this.towerId]) { //sélectionne une tour dans la grille
                ...
                guideline.textContent = "click on a new tower\r\n\r\nor\r\n\r\nselect a tower on the grid\r\n\r\nor\r\n\r\nclick on launch";
            }
            //selectedTower = towers[this.towerId];
            //displayDatas(this.towerId, false);
            //selectedTower.isSelected = true;
...
function createTower() {
    ...
    towerImageRadius = 75 + (10 * pickedTowerType);
...
function positionTower(e) {
    ...
    let towerImage = document.getElementById("towerImage");
    let towerImageRadius = 75 + (10 * pickedTowerType);
    let towerColorHSL = types[pickedTowerType];
    let towerColor = "hsla(" + towerColorHSL + ", 100%, 50%, .15)";
    document.documentElement.style.setProperty('--pickedTowerDotColor', "hsla(" + towerColorHSL + ", 100%, 50%, .75)");
        ...
        if (towerType == pickedTowerType) {
           ...
           // box.style.background = "rgba(255, 25, 25, .75)";
            ...
            if (score >= towersCost[towerType]) {
                ...
                guideline.textContent = "place the tower on the grid";
                ...
                guideline.textContent = "not enough credits";
                if (resetDisplay) {
                    clearTimeout(resetDisplay);
                }
...
class Tower {
    ...
    updateOptions() { // [nom, description, valeur, durée effet, temps de recharge, url icone] *** FAIT: [0][0];[0][1];[2][0]
        ...
        switch(this.towerType) {
            case 0:
                ...
                this.effects = [
                    ["freezing", "slows down target by "+ (100 - this.speedMultiplier * 100 )+"% for "+ this.effectDuration / (fps / 4) +" second" + singOrPlur, this.speedMultiplier, this.effectDuration, this.effectCooldown, "url(./image/ice_crystal.png)"],
                    ["frost mastery", "damage up by 100%, heal reduced by 75%"]]
            break;
            case 1:
                ...
                this.effects = [["flooding", "ice & water mobs : slows down target by "+ (100 - this.speedMultiplier * 100 )+"%\r\nfire & lightning mobs : deals "+ this.damage * 10 * this.burnMultiplier +" damage\r\nduration : " + this.effectDuration / (fps / 4) +" second" + singOrPlur, this.speedMultiplier, this.effectDuration, this.effectCooldown, "url(./image/water_drop.png)"],
                ["flooding mastery", "damage up by 100%, heal reduced by 75%"]]
            break;
            case 2:
                ...
                this.effects = [["combustion", "burns target for "+ this.damage * 10 * this.burnMultiplier +" damage in "+ this.effectDuration / (fps / 4) +" second" + singOrPlur, this.damage * 20, this.effectDuration, this.effectCooldown, "url(./image/fire_flame.png)"],
                ["conflagrations mastery", "damage up by 100%, heal reduced by 75%"]]
            break;
            case 3:
                this.effectDuration = 30;
                this.effectCooldown = 90;
                ...
                this.effects = [["shock", "shocks target, causing it to stop for "+ this.effectDuration / (fps / 4) +" second"+ singOrPlur, null, this.effectDuration, this.effectCooldown, "url(./image/lightning_lightning.png)"],
                ["voltage mastery", "damage up by 100%, heal reduced by 75%"]]
            break;
            case 4:
                this.effects = [["assimilation", "drains target life, turning the damage done to credits", null, null, null, "url(./image/dark_dark.png)"],
                ["dark mastery", "damage up by 100%, heal reduced by 75%"]]
            break;
            default:
    ...
    draw() {
        ...
        ctx.arc(this.x, this.y, this.radius / 32, 0, Math.PI * 2);
        ctx.closePath();
        ctx.fill();
...
/*         else {
        ...
        switch(option) {
            ...
            break;
            case 1:
                scoreUpdateBoxUpdate("upgrade tower ", this.towerId);
                switch(this.towerType) {
                    case 0:
                        this.speedMultiplier = this.effects[0][2];
                        this.slowDuration = this.effects[0][3];
                    break;
                    case 1:
                        this.burnMultiplier = 1;
                        this.speedMultiplier = this.effects[0][2];
...
/*         if (!this.hasOptionSpec1) {
        ...
        if (this.isSelected) {
            //levelUpCost.textContent = this.optionLevelUpCost.toLocaleString("en-US");
            optionSpec1.style.backgroundImage = this.effects[0][5]/*  + " center center" */;
    ...
    shot() {
        ...
        if (this.target != undefined && this.target.hp > 0) {
            ...
            if (mobs[this.target.id].previousHp > mobs[this.target.id].hp) {
                mobs[this.target.id].color = 0;
            }
            else if (mobs[this.target.id].previousHp < mobs[this.target.id].hp) {
                mobs[this.target.id].color = 120;
            }
            else {
                mobs[this.target.id].color = mobs[this.target.id].baseColor;
            }
        }
        ...
        if (this.hasOptionSpec1 && !this.optionSpec1IsOnCoolDown) {  //spec1
            ...
            let shotCooldown;  //parfois, faux message d'erreur qui dit que la variable n'est jamais lue alors qu'elle est lue dans le switch
            let waveType;
            switch(this.towerType) {
                case 0: //ice tower
                    ...
                    if (!this.target.isFrozenSlowed) {
                        ...
                        this.target.frozenSlowDuration = this.effects[0][3];
                    ...
                    if (!this.target.isFlooded) {
                        ...
                        switch(waveType) {
                            ...
                            case 1:
                                ...
                                this.target.speed *= this.speedMultiplier;
                                this.target.floodSlowDuration = this.effects[0][3];
                            ...
                            case 3:
                                ...
                                this.target.floodBurnDuration = this.effects[0][3];
                                this.floodBurnTickInterval = 15;
                                this.target.floodBurnTickInterval = this.burnTickInterval;
                                this.target.floodBurnNextTickInterval = this.target.burnTickInterval;
                                this.target.floodBurnTicksLeft = this.effects[0][3] / this.burnTickInterval;
...
/*                         this.target.burnOrigin = this.towerId;
                ...
                case 3: //electric tower
                    ...
                    if (!this.target.isShocked) {
                        ...
                        this.speedMultiplier = 0;
                        this.target.speed *= this.speedMultiplier;
                        this.target.shockDuration = this.effects[0][3];  // 30 frames
                    }
                break;
                case 4: //dark tower
                    waveType = waveSettings - 1;
            ...
            this.optionSpec1CooldownDuration = this.effects[0][4];
        }
    }
}
...
class Mob {
    ...
    draw() {
       ...
       // this.radius = 
        ...
        ctx.beginPath();
        ctx.arc(this.x * cellSize, this.y * cellSize, size, 0, Math.PI * 2);
        ctx.closePath();
       // ctx.fillStyle = "hsl(" + this.color + ", 100%, 50%)";
        ctx.fill();
        ctx.beginPath();
        ctx.arc(this.x * cellSize, this.y * cellSize, radius * 2, 0, Math.PI * 2);
        ctx.closePath();
        ctx.strokeStyle = "hsl(" + this.color + ", 100%, 50%)";
        ctx.stroke();
        ctx.shadowBlur = 0;
                ...
                case 3:
                    if (this.floodBurnDuration == 0) {
                        ...
                        if (this.floodBurnNextTickInterval == 0) {
                            ...
                            if (this.hp < 0) {
                                towers[this.floodBurnOrigin].damageDone += this.hp;
                            }
        ...
        if (this.isBurning) {
            if (this.burnDuration == 0) {
                ...
                if (this.burnNextTickInterval == 0) {
                    ...
                    if (this.hp < 0) {
                        towers[this.burnOrigin].damageDone += this.hp;
                    }
...
const mobsBaseHp = 100; // valeur divisé par 10 pour la première vague
let waveNumber = 0;
...
let waveId = document.getElementById("waveId");
...
function waveInit() {
    if (gameIsOver) {
       ...
       // mobsTypeNext.style.boxShadow = "0 0 10px hsl(" + types[1] + " , 100%, 50%)";
        ...
        mobsHp.textContent = mobs[0].hp.toLocaleString("en-US");
        //updateGrid();
        // HERE2    
...
let creditsMultiplier = 3;  //valeur de base !modifiable!
...
/*     if (timeStamp == 0) {
    ...
    if (mobs.length == 0 && spawnSequence == false && gameIsOver == false) {
...
/*             document.getElementById("scoreUpdate").remove();
        ...
        launchText.textContent = "victory";
...
function resize(width, height, context) {
    console.log("RESIZE; gridSize= "+ gridSize);
    context.width = width;
    ...
    console.log("RESIZE; cellSize= "+ cellSize);
}
...
function generateLevelThumbnail() {
    ...
    for (let i = 0; i < levelsNumber; i++) {
        ...
        levelsPickContainer.appendChild(levelPickDiv);
    }
}
...
function reset() {
    if (!confirmReset) {
        ...
        quitButton.textContent = "confirm";
        confirmReset = true;
    }
    else {
        ...
        topPlayersTitle.textContent = "top scores";
...
let userHasPickedALevel = false; //here passer sur true pour édition de niveau
function pickLevel(id) {
    ...
    topPlayersTitle.textContent = levelsName[id] + " - top scores";
    previewLevel(id);
    showInterface();
}
...
const waveDatasBox = document.querySelectorAll("div.wave-datas");
const levelDatasBox = document.getElementById("levelDatas");
const levelName = document.getElementById("levelName");
const playersList = document.getElementById("playersList");
const topPlayersTitle = document.getElementById("topPlayersTitle");
function hideInterface() {
    for (let i = 0; i < waveDatasBox.length; i++) {
        ...
        for (let j = 0; j < nodes.length; j++) {
            console.log("node "+j+" ");
            console.log(nodes[j]);
            if (nodes[j].className == "title" || nodes[j].className == "row") {
...
function checkScore() {
    ...
    if (score < 0) {  // affichage de l'écran de fin de partie
        gameIsOver = true;
        //launchText.style.letterSpacing = ".05em";
        launchText.textContent = "defeat";
        setTimeout(() => {
            let scoreUpdateBox = document.getElementById("scoreUpdate");
            ...
            waveProgressBar.classList.add("loading-5s");
            scoreUpdateBox.appendChild(waveProgressBar);
        }, 10);
...
function displayDatas(itemId, isNew) {
    ...
    if (isNew){
        itemName.textContent = towersName[itemId] + " tower";
        ...
        selectedItemRadius.textContent = 75 + (10 * itemId);
        ...
        selectedItemId.textContent = "stats for tower #" + itemId;
        ...
        itemName.textContent = towersName[towers[itemId].towerType] + " tower #" + selectedTower.towerId;
        ...
        selectedItemRadius.textContent = towers[itemId].radius;
...
const selectedItemOptionsDisplay = document.getElementById("selectedItemOptionsDisplay");
...
function hoverOptions(e) {
    ...
    switch(target) {
        case "optionLevelUp":
            selectedItemOptionsDisplay.textContent = "level up";
            tooltipEffect.textContent = "add 1 level to the tower";
            tooltipCooldown.textContent = "next level stats : ";
            positionTooltip(e);
            break;
        case "optionSpec1":
            ...
            selectedItemOptionsDisplay.textContent = towers[selectedTower.towerId].effects[0][0];
            tooltipEffect.textContent = towers[selectedTower.towerId].effects[0][1];            
            if (towers[selectedTower.towerId].effects[0][4] / 30  < 2) {
                singOrPlur = "";
            }
            else {
                singOrPlur = "s";
            ...
            if (selectedTower.towerType != 4) {
                tooltipCooldown.textContent = "cooldown : " + towers[selectedTower.towerId].effects[0][4] / 30 + " second" + singOrPlur;
            }
            positionTooltip(e);
            break;
        case "optionSpec2":
            selectedItemOptionsDisplay.textContent = towers[selectedTower.towerId].effects[1][0];
            tooltipEffect.textContent = towers[selectedTower.towerId].effects[1][1];
            if (towers[selectedTower.towerId].effects[0][4] / 30  < 2) {
                singOrPlur = "";
            }
            else {
            ...
            break;
        case "optionDestroy":
            selectedItemOptionsDisplay.textContent = "destroy";
            tooltipEffect.textContent = "destroy the tower";
            if (towers[selectedTower.towerId].effects[0][4] / 30  < 2) {
                singOrPlur = "";
            }
            else {
            ...
            tooltipCooldown.textContent = "warning: no refund";
            positionTooltip(e);
            break;
        default:
            selectedItemOptionsDisplay.textContent = "hover an upgrade";
            hideTooltip();
            break;
    }
}
...
window.addEventListener("click", function(e) {
    ...
    if (!userHasPickedALevel) {
        guideline.textContent = "select a map";
    }
...
function adjustGameSize() {
    ...
    if (window.innerHeight < canvas.height * 1.5) {
        ...
        console.log("adjust to "+ (newSize / 1.5));
        gridSizes = (newSize / 1.5);
        canvas.width = gridSizes;
        canvas.height = gridSizes;
        grid.width = gridSizes;
        grid.height = gridSizes;
